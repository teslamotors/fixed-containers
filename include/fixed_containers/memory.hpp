...
template <typename T>
const std::byte* addressof_as_const_byte_ptr(T& p)
{
    return reinterpret_cast<const std::byte*>(std::addressof(p));
}
template <typename T>
std::byte* addressof_as_mutable_byte_ptr(T& p)
{
    return reinterpret_cast<std::byte*>(std::addressof(p));
}
