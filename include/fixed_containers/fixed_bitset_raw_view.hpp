...
{
    constexpr std::size_t UL_BYTES = sizeof(uint32_t);
    constexpr std::size_t UL_BITS = UL_BYTES * CHAR_BIT;
    constexpr std::size_t ULL_BYTES = sizeof(uint64_t);
    return max_element_count <= UL_BITS ? UL_BYTES : ULL_BYTES;
}
...
public:
    ...
    [[nodiscard]] constexpr std::size_t storage_size() const { return get_storage_size(max_elem_count_); }
    [[nodiscard]] constexpr std::size_t get_alignment() const noexcept
    {
        return get_storage_word_size(max_elem_count_);
    }
