...
{
    {
        ...
        EXPECT_EQ(0,
                  path_properties.at(path_from_string("ignore1_dont_forget_alignment"))
                      .offset.base_offset);
        EXPECT_EQ(struct_view_detail::AS_PRIMITIVE,
                  path_properties.at(path_from_string("ignore1_dont_forget_alignment"))
...
TEST(StructView, SubStructViewOfEnum)
{
    ...
    // check field properties
    EXPECT_EQ(struct_view_detail::AS_PRIMITIVE_ENUM,
              super_struct_view.at(path_from_string("enum_field")).metadata.metadata_type);
    EXPECT_EQ(struct_view_detail::AS_PRIMITIVE_STRING_VIEW,
...
inline constexpr std::size_t TEST_VECTOR_RESIZED_SIZE = 2;
...
TEST(StructView, SubStructViewOfRecursiveWithArray)
{
    ...
    // test that resizing will
    array_test_super_struct_1.vec.resize(TEST_VECTOR_RESIZED_SIZE);
    for (auto&& element : array_test_super_struct_1.vec)
    {
        element.vec.resize(TEST_VECTOR_RESIZED_SIZE);
    }
    ...
    for (std::size_t i = 0; i < TEST_ARRAY_SIZE; ++i)
    {
        ...
        // resizable part
        if (i < array_test_super_struct_1.vec.size())
        {
            ASSERT_EQ(array_test_sub_struct_1.vec[i].vec.size(),
                      array_test_super_struct_1.vec[i].vec.size());
            for (std::size_t j = 0; j < array_test_sub_struct_1.vec[i].vec.size(); ++j)
            {
...
TEST(ExtensibleReflectionTests, CArray)
{
    ...
    const StructView struct_view(c_array);
