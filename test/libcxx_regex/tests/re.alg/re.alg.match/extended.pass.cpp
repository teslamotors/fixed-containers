...
// Ported from LLVM 242ccbf6b6d9d090762a87138c854f2d93b6ab60/re.alg/re.alg.match/extended.pass.cpp
...
{
    {
        ...
        const char s[] = "a";
        assert(libcxx_fixed_regex::regex_match(s, m, libcxx_fixed_regex::regex("a", std::regex_constants::extended)));
        assert(m.size() == 1);
        assert(!m.empty());
    ...
    }
    {
        libcxx_fixed_regex::cmatch m;
        const char s[] = "ab";
        assert(libcxx_fixed_regex::regex_match(s, m, libcxx_fixed_regex::regex("ab", std::regex_constants::extended)));
        assert(m.size() == 1);
        assert(!m.prefix().matched);
        assert(m.prefix().first == s);
        assert(m.prefix().second == m[0].first);
    ...
    }
    {
        libcxx_fixed_regex::cmatch m;
        const char s[] = "ab";
        assert(!libcxx_fixed_regex::regex_match(s, m, libcxx_fixed_regex::regex("ba", std::regex_constants::extended)));
        assert(m.size() == 0);
        assert(m.empty());
    }
    {
    ...
    }
    {
        libcxx_fixed_regex::cmatch m;
        const char s[] = "abcd";
        assert(!libcxx_fixed_regex::regex_match(s, m, libcxx_fixed_regex::regex("bc", std::regex_constants::extended)));
        assert(m.size() == 0);
    }
    {
        libcxx_fixed_regex::cmatch m;
        const char s[] = "abbc";
        assert(libcxx_fixed_regex::regex_match(s, m, libcxx_fixed_regex::regex("ab*c", std::regex_constants::extended)));
        assert(m.size() == 1);
        assert(!m.prefix().matched);
        assert(m.prefix().first == s);
        assert(m.prefix().second == m[0].first);
    ...
    }
    {
        libcxx_fixed_regex::cmatch m;
        const char s[] = "abc";
        assert(libcxx_fixed_regex::regex_match(s, m, libcxx_fixed_regex::regex("^abc", std::regex_constants::extended)));
        assert(m.size() == 1);
        assert(!m.prefix().matched);
        assert(m.prefix().first == s);
        assert(m.prefix().second == m[0].first);
    ...
    }
    {
        libcxx_fixed_regex::cmatch m;
        const char s[] = "abc";
        assert(libcxx_fixed_regex::regex_match(s, m, libcxx_fixed_regex::regex("a.c", std::regex_constants::extended)));
        assert(m.size() == 1);
        assert(!m.prefix().matched);
        assert(m.prefix().first == s);
        assert(m.prefix().second == m[0].first);
    ...
    }
    {
        libcxx_fixed_regex::cmatch m;
        const char s[] = "abcdef";
        assert(libcxx_fixed_regex::regex_match(s, m, libcxx_fixed_regex::regex("(.*).*", std::regex_constants::extended)));
        assert(m.size() == 2);
        assert(!m.prefix().matched);
        assert(m.prefix().first == s);
        assert(m.prefix().second == m[0].first);
    ...
    }
    {
        libcxx_fixed_regex::cmatch m;
        const char s[] = "abbc";
        assert(!libcxx_fixed_regex::regex_match(s, m, libcxx_fixed_regex::regex("ab{3,5}c", std::regex_constants::extended)));
        assert(m.size() == 0);
    }
    {
        libcxx_fixed_regex::cmatch m;
    ...
    }
    {
        libcxx_fixed_regex::cmatch m;
        const char s[] = "adec";
        assert(!libcxx_fixed_regex::regex_match(s, m, libcxx_fixed_regex::regex("a.{3,5}c", std::regex_constants::extended)));
        assert(m.size() == 0);
    }
    {
        libcxx_fixed_regex::cmatch m;
    ...
    }
    {
        libcxx_fixed_regex::cmatch m;
        const char s[] = "-ab,ab-";
        assert(!libcxx_fixed_regex::regex_match(s, m, libcxx_fixed_regex::regex("-(.*),\1-", std::regex_constants::extended)));
        assert(m.size() == 0);
    }
    {
        libcxx_fixed_regex::cmatch m;
...
#ifndef TEST_HAS_NO_WIDE_CHARACTERS
    {
        std::wcmatch m;
        const wchar_t s[] = L"a";
        assert(libcxx_fixed_regex::regex_match(s, m, std::wregex(L"a", std::regex_constants::extended)));
        assert(m.size() == 1);
        assert(!m.empty());
        assert(!m.prefix().matched);
        assert(m.prefix().first == s);
    ...
    }
    {
        ...
        assert(libcxx_fixed_regex::regex_match(s, m, std::wregex(L"ab", std::regex_constants::extended)));
        assert(m.size() == 1);
        assert(!m.prefix().matched);
        assert(m.prefix().first == s);
        assert(m.prefix().second == m[0].first);
    ...
    }
    {
        std::wcmatch m;
        const wchar_t s[] = L"ab";
        assert(!libcxx_fixed_regex::regex_match(s, m, std::wregex(L"ba", std::regex_constants::extended)));
        assert(m.size() == 0);
        assert(m.empty());
    }
    {
    ...
    }
    {
        ...
        assert(!libcxx_fixed_regex::regex_match(s, m, std::wregex(L"bc", std::regex_constants::extended)));
        assert(m.size() == 0);
    }
    {
        std::wcmatch m;
        const wchar_t s[] = L"abbc";
        assert(libcxx_fixed_regex::regex_match(s, m, std::wregex(L"ab*c", std::regex_constants::extended)));
        assert(m.size() == 1);
        assert(!m.prefix().matched);
        assert(m.prefix().first == s);
        assert(m.prefix().second == m[0].first);
    ...
    }
    {
        ...
        assert(libcxx_fixed_regex::regex_match(s, m, std::wregex(L"^abc", std::regex_constants::extended)));
        assert(m.size() == 1);
        assert(!m.prefix().matched);
        assert(m.prefix().first == s);
        assert(m.prefix().second == m[0].first);
    ...
    }
    {
        std::wcmatch m;
        const wchar_t s[] = L"abc";
        assert(libcxx_fixed_regex::regex_match(s, m, std::wregex(L"a.c", std::regex_constants::extended)));
        assert(m.size() == 1);
        assert(!m.prefix().matched);
        assert(m.prefix().first == s);
        assert(m.prefix().second == m[0].first);
    ...
    }
    {
        ...
        assert(libcxx_fixed_regex::regex_match(s, m, std::wregex(L"(.*).*", std::regex_constants::extended)));
        assert(m.size() == 2);
        assert(!m.prefix().matched);
        assert(m.prefix().first == s);
        assert(m.prefix().second == m[0].first);
    ...
    }
    {
        std::wcmatch m;
        const wchar_t s[] = L"abbc";
        assert(!libcxx_fixed_regex::regex_match(s, m, std::wregex(L"ab{3,5}c", std::regex_constants::extended)));
        assert(m.size() == 0);
    }
    {
        std::wcmatch m;
    ...
    }
    {
        ...
        assert(!libcxx_fixed_regex::regex_match(s, m, std::wregex(L"a.{3,5}c", std::regex_constants::extended)));
        assert(m.size() == 0);
    }
    {
        std::wcmatch m;
    ...
    }
    {
        ...
        assert(!libcxx_fixed_regex::regex_match(s, m, std::wregex(L"-(.*),\1-", std::regex_constants::extended)));
        assert(m.size() == 0);
    }
    {
        std::wcmatch m;
