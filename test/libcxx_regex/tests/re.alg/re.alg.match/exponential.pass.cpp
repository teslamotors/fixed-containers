...
// Ported from LLVM 242ccbf6b6d9d090762a87138c854f2d93b6ab60/re.alg/re.alg.match/exponential.pass.cpp
...
int main(int, char**) {
  for (std::regex_constants::syntax_option_type op :
    ...
    try {
      bool b = libcxx_fixed_regex::regex_match(
  ...
  for (std::regex_constants::syntax_option_type op :
       {libcxx_fixed_regex::regex::ECMAScript, libcxx_fixed_regex::regex::extended, libcxx_fixed_regex::regex::egrep,
        libcxx_fixed_regex::regex::awk}) {
    assert(libcxx_fixed_regex::regex_match(s, libcxx_fixed_regex::regex("a*", op)));
  }
  return 0;
}
